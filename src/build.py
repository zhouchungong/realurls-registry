...
def write_sql(path: Path, entities: list[dict], index: dict, verified: list[str], version: str) -> None:
    ...
    stmts = [
        # Aggregate query demand (no IP, no user agent, no session: day + query key + verdict + count). Lives
        ...
        "DROP INDEX IF EXISTS idx_domains_official; DROP INDEX IF EXISTS idx_entities_name; DROP INDEX IF EXISTS idx_domains_new_entity; "
