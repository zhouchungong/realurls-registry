...
| **A6** | Propagation | 0.65 | source domain is itself `verified`, **and the source's page links to this domain (first-party declaration)**, **and** ≥1 structural link; with `cert_san` alone, SAN count ≤ 25; **and** this domain's page is seen linking back to the source (unknown counts as no) unless the source's own certificate covers this domain; shared name servers count only when they are not a provider's defaults (Namecheap, GoDaddy, Google, Hurricane Electric… hand every customer the same pair), **and** no other GitHub organization declares this domain as its site, **and** the domain's own Wikidata item, if any, is not an organization (a product item such as MediaWiki does not count; a newspaper or foundation does) |
...
**A3 never anchors on its own, and A7 does not either once the entity has an identity of its own (a Wikidata item or GitHub organization).** They say that *some* substantial corporation, or *some* government body, holds the domain, not which. They count only when at least one piece of evidence that names the entity also passed (A1, A2, A4, A5, A6, A8, A9 or B1). Without that rule, A3 + B4 + B5 would verify any famous corporate domain under any entity we happened to ask about; in the first full-category batch that is exactly what happened to anthropic.com (under the Python project), digitalocean.com (under five projects it sponsors) and vmware.com (under RabbitMQ).
...
5. **Single-record entry points** (`src/examine.py`): a domain someone asked about (API, MCP or the site; examined within about fifteen minutes), a community lead (issue), or an owner self-attestation (issue) all go through the same pipeline and the same writer. A record that reaches `verified` is merged by the bot after the adversarial corpus and the AI review pass, without waiting for a batch; it joins the pool the next manual sample is drawn from. Batches (thousands of seeds) additionally wait for the manual sample before merging. Anything less is recorded as *examined* with the rules' reasons, so the API answers "examined on this date, insufficient evidence" rather than "never looked".
