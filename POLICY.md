...
| **A6** | Propagation | 0.65 | source domain is itself `verified`, **and the source's page links to this domain (first-party declaration)**, **and** ≥1 structural link; with `cert_san` alone, SAN count ≤ 25; **and** this domain's page is seen linking back to the source (unknown counts as no) unless the source's own certificate covers this domain; shared name servers count only when they are not a provider's defaults (Namecheap, GoDaddy, Google, Hurricane Electric… hand every customer the same pair), **and** no other GitHub organization declares this domain as its site |
