...
| **A4** | TLS certificate Subject `O=` | 0.70 | **OV/EV** certificate with a non-empty `O=` |
| **A6** | Propagation | 0.65 | source domain is itself `verified`, **and the source's page links to this domain (first-party declaration)**, **and** ≥1 structural link; with `cert_san` alone, SAN count ≤ 25 |
