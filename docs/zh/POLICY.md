...
> 中文镜像，供参考；**以仓库根目录的[英文版](../../POLICY.md)为准**（[英文版](../../POLICY.md)随代码同步更新，本页可能滞后）。
...
| **A9** | 应用商店史 | 0.55 | 一个卖家 URL 指向本域名的应用，由 Apple 核验过法人身份的公司发布，卖家名或应用名与已锚定实体一致，且上架 **不少于 2 年、不少于 1,000 评分**。A8 的商业版：Apple 核验卖家法人，上架年限买不到 |
| **A4** | 证书 Subject `O=` | 0.70 | 证书为 **OV/EV**，且 `O=` 就是**已锚定的实体**（标签或 canonical GitHub 组织，忽略公司后缀）。别家的 OV 证书只证明控制权，不证明归属 |
| **A6** | 锚点扩散 | 0.65 | 源域名自身 `verified` **且 源域名页面链接到本域名（一方声明）** **且** ≥1 条结构性关联；仅靠 `cert_san` 时 SAN 数 ≤ 25；**且** 本域名页面若有出站链接，必须链回源域名 |
...
2. **人工抽样审计** —— 每次数据集发布前抽 200 条 `verified` 人工复核。低于 99.5% 则回退规则，不许发布。抽样可复现（`python -m src.audit_sample draw --batch <name>`），填好的清单提交到 `audits/` 供任何人复查，`score` 算出用于决策的那个数字。
3. **每修一个误报，必须往负样本语料里加一条对应用例。**
5. **按需检查**（`src/examine.py`，每十分钟）：有人查过而我们从未检查的域名，走同一条流水线。达到 `verified` 的记录在负样本回归和 AI 复核通过后由 bot 直接合入，不等批次；它们进入下一次人工抽样的池子。未达到的记为"已检查"并附规则原因，API 回答"某日已检查，证据不足"而不是"从没看过"。
4. **AI 复核，第二道审计**（`src/review_ai.py`）：每条 `verified` 记录都会交给一个语言模型，只问一个问题——已存证据里有没有迹象表明这个域名**不**属于这个实体？被标记的记录转为 `review_required` 并保持，直到有人清除。**这一层只能撤销 verified，永远不能授予。** 没有任何记录会因为模型说了算而变成 `verified`——上面的计数规则仍是唯一路径。第 2 条的人工抽样正是用来证明这一层本身可信的，所以抽样规模不随数据量缩减。
