...
权威 ③：Wikidata 条目站点链接不足 3 个时，可由 App Store 开发者身份补足——卖家 URL 指向本域名、Apple 已核验法人的卖家法定名称与条目标签一致、上架 ≥2 年且 ≥1,000 评分（即 A9 门槛）。Wikidata 编辑者与 Apple 互不相关，两者在同一域名上指认同一组织，这是身份而不只是控制权。第二个 Wikidata 批次有 4,427 家公司卡在站点链接门槛之下，其中 127 家是成熟的 App Store 开发者（多为游戏工作室），权威 ③ 为此而设。
...
| **A6** | 锚点扩散 | 0.65 | 源域名自身 `verified` **且 源域名页面链接到本域名（一方声明）** **且** ≥1 条结构性关联；仅靠 `cert_san` 时 SAN 数 ≤ 25；**且** 必须实际看到本域名页面链回源域名（看不到按"否"计），除非源域名自己的证书覆盖本域名；共享 NS 仅在不是服务商默认 NS 时才算数（Namecheap、GoDaddy、Google、HE 给每个客户的都是同一对），**且** 没有别的 GitHub 组织把本域名声明为自己的站点，**且** 本域名自己的 Wikidata 条目（若有）不是组织类（MediaWiki 这类产品条目不算，报社、基金会算） |
...
5. **单条入口**（`src/examine.py`）：有人查询（API、MCP 或站点，约十五分钟内检查）、社区线索（issue）、所有者自证（issue），三者走同一条流水线、同一个写入者。达到 `verified` 的记录在负样本回归和 AI 复核通过后由 bot 直接合入，不等批次；它们进入下一次人工抽样的池子。批次（数千种子）合并前还要等人工抽样。未达到的记为"已检查"并附规则原因，API 回答"某日已检查，证据不足"而不是"从没看过"。
