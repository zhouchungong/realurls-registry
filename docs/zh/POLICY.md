...
| **A6** | 锚点扩散 | 0.65 | 源域名自身 `verified` **且 源域名页面链接到本域名（一方声明）** **且** ≥1 条结构性关联；仅靠 `cert_san` 时 SAN 数 ≤ 25；**且** 必须实际看到本域名页面链回源域名（看不到按"否"计），除非源域名自己的证书覆盖本域名；共享 NS 仅在不是服务商默认 NS 时才算数（Namecheap、GoDaddy、Google、HE 给每个客户的都是同一对），**且** 没有别的 GitHub 组织把本域名声明为自己的站点 |
