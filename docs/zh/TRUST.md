...
社区提交 → Issue Form（只提交「线索」）
        → 机器人自动跑全套证据采集
        → 证据充分：机器人直接合入（须通过负样本回归 + AI 复核）
        → 任何人通过 API 查询的域名，几分钟内以同样方式检查
        → 证据不足：自动关闭，并告诉你缺哪条
```
...
| 结果 | 存在哪里 | 如何保鲜 | 谁会知道 |
|---|---|---|---|
| `verified` | git 里的 `entities/*.yaml` → D1 → 签名 release | 每日重验；变化写入 `history` | 提出它的 issue（附上线链接后关闭）；API/MCP/站点即时生效 |
| 已检查但未通过 | `examined` 表（日期、状态、原因） | 30 天后再被问到时重查，或所有者流程即时重查 | API/MCP/站点回答"某日已检查"；提出它的 issue 收到原因 |
| 重验降级 | 记录的 `history`；维护者 issue | 每日 | 所有者在自己的 issue 上收到通知；使用者看到答案变化 |
| 争议 | 记录上的 `dispute` 保持 | 保持到人工清除并公开说明 | 争议 issue，几分钟内 |
| 规则变更 | POLICY.md、CHANGELOG.md、git 历史 | 全量重验并写明对记录的影响 | 所有人，在 changelog 里 |
